...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.43', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
st.title('Extrator DIRF + Motor Previdenciário V1.2.43')
...
bytes_caso_atual = json.dumps(payload_caso_atual, ensure_ascii=False, indent=2).encode('utf-8')
...
with tabs[4]:
    ...
    if ap_base.empty:
        ...
        # Controle de escopo por competência. A competência continua presente
        # no RAW e pode ser reativada a qualquer momento, mas quando inativada
        # deixa de participar da apuração e dos relatórios consolidados.
        if not ap_scope.empty:
            ...
            # A seleção é persistente no caso, inclusive quando o usuário
            # muda temporariamente o intervalo da apuração.
            ...
            st.markdown('### 4. Competências consideradas na apuração')
            st.caption('Desmarque a caixa de uma competência para **inativá-la**. Ela permanecerá nos dados extraídos, mas não será considerada no cálculo, nos totais ou no relatório consolidado. O relatório técnico registrará quais competências foram inativadas.')
            colunas_comp = st.columns(4)
            for idx, label_comp in enumerate(labels_scope):
                col = colunas_comp[idx % 4]
                chave = f'apuracao_inativar_{re.sub(r"[^A-Za-z0-9]", "_", label_comp)}'
                marcado_inativo = col.checkbox(
                    f'Inativar {label_comp}',
                    value=(label_comp in inativadas),
                    key=chave,
                )
                if marcado_inativo:
                    inativadas.add(label_comp)
                else:
                    inativadas.discard(label_comp)
            st.session_state.apuracao_competencias_inativadas = inativadas

            ativas_scope = [x for x in labels_scope if x not in inativadas]
            if inativadas:
                st.warning('Competências inativadas: ' + ', '.join(sorted(inativadas, key=lambda x: _ord_ap_item(comp_to_item.get(x, ('9999-99', 'mensal'))))) + '. Elas não participarão da apuração.')
            else:
                st.success('Todas as competências do escopo estão ativas para a apuração.')
            ...
            st.info('Não há competências no escopo para ativar/inativar.')
            ...
            if result.empty:
                ...
                for _, rr in result.iterrows():
                    ...
                    resumo_ap.append({
                        ...
                        'Rem. progressiva': rr.get('remuneracao_progressiva', 0),
                        'Máx. progressiva': rr.get('contribuicao_maxima_progressiva', 0),
                        'Rem. 11%': rr.get('remuneracao_11', 0),
                        'Máx. 11%': rr.get('contribuicao_maxima_11', 0),
                        'Rem. 20%': rr.get('remuneracao_20', 0),
                        'Saldo p/ 20%': rr.get('saldo_teto_apos_11', 0),
                ...
                st.dataframe(resumo_ap_df, use_container_width=True, hide_index=True, height=780, column_config={
                    c: st.column_config.NumberColumn(c, format='R$ %.2f')
                    for c in ['Recolhido','Teto','Rem. progressiva','Máx. progressiva','Rem. 11%','Máx. 11%','Rem. 20%','Saldo p/ 20%','Máx. 20%','Máx. total','Acima do teto']
                })
...
with tabs[6]:
    ...
    st.download_button('🗂️ Baixar JSON RAW somente', json.dumps({'schema': 'extrator_dirf.v3.3', 'registros': records}, ensure_ascii=False, indent=2).encode('utf-8'), 'extracao_dirf_raw.json', 'application/json')
    if checks:
